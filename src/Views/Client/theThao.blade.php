...
<div class="page_header clearfix page_margin_top">
    <div class="page_header_left">
        <h1 class="page_title">Tin Thể Thao</h1>
    </div>
...
<div class="page_layout clearfix">
    ...
    <div class="row">
        <div class="column column_2_3">
            <div class="row">
                <ul class="blog big">
                    <?php foreach ($tinTheThao as $tt) {?>
                        <li class="post">
                            <a href="{{ url("client/{$tt['id']}/{$tt['idDirectory']}/show") }}" title="Built on Brotherhood, Club Lives Up to Name">
                                <img src='{{show_upload($tt['image'])}}' alt='img' >
                            </a>
                            <div class="post_content">
                                <h2 class="with_number">
                                    <a href="{{ url("client/{$tt['id']}/{$tt['idDirectory']}/show") }}" title="Built on Brotherhood, Club Lives Up to Name">{{$tt['title']}}</a>
                                ...
                                <ul class="post_details">
                                    <li class="category"><a href="index7855.html?page=category&amp;cat=world" title="WORLD">Thể Thao</a></li>
                                    <li class="date">
                                        {{$tt['created_at']}}
                                    </li>
                                </ul>
                                ...
                                overflow: hidden;">{{$tt['content']}}</p>
                                <a class="read_more" href="{{ url("client/{$tt['id']}/{$tt['idDirectory']}/show") }}" title="Read more"><span class="arrow"></span><span>XEM THÊM</span></a>
                            </div>
                        </li>
                        <?php } ?>
                </ul>
        ...
        <div class="column column_1_3 page_margin_top">
            ...
            <div class="vertical_carousel_container clearfix">
                <ul class="blog small vertical_carousel autoplay-1 scroll-1 navigation-1 easing-easeInOutQuint duration-750">
                    @for ($i = 0; $i < 4; $i++)
                    <li class="post">
                        <a href="{{ url("client/{$tinTheThao[$i]['id']}/{$tinTheThao[$i]['idDirectory']}/show") }}" title="Study Linking Illnes and Salt Leaves Researchers Doubtful">
                            <span class="icon small gallery"></span>
                            <img src='{{show_upload($tinTheThao[$i]['image'])}}' alt='img' style="width: 100px">
                        </a>
                        <div class="post_content">
                            <h5>
                                <a href="{{ url("client/{$tinTheThao[$i]['id']}/{$tinTheThao[$i]['idDirectory']}/show") }}" title="Study Linking Illnes and Salt Leaves Researchers Doubtful">{{$tinTheThao[$i]['title']}}</a>
                            </h5>
                            <ul class="post_details simple">
                                <li class="category"><a href="indexdced.html?page=category&amp;cat=health" title="HEALTH">Thể Thao</a></li>
                                <li class="date">
                                    {{$tinTheThao[$i]['created_at']}}
                                </li>
                            </ul>
                        </div>
                    </li>
